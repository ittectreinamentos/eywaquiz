...
const Signup = () => {
  ...
  const Field = ({ label, field, type = "text", placeholder, value, onChange }: {
    label: string; field: string; type?: string; placeholder: string; value: string; onChange: (v: string) => void;
  }) => (
    <div className="space-y-1">
      ...
      <Input
        ...
        className={`bg-card border-border text-foreground placeholder:text-muted-foreground ${errors[field] ? "border-destructive" : ""}`}
      />
  ...
  return (
    <motion.div className="fixed inset-0 bg-background bg-gradient-dark flex flex-col overflow-y-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <Particles count={6} />
      <div className="relative z-10 w-full max-w-sm mx-auto px-6 py-8 space-y-5">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/login")} className="p-1.5 rounded-lg bg-muted hover:bg-muted/80 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex items-start gap-2 pt-1">
            <Checkbox checked={acceptTerms} onCheckedChange={(v) => { setAcceptTerms(!!v); setErrors((e) => ({ ...e, terms: "" })); }} className="mt-0.5" />
            <label className="text-muted-foreground text-[11px] leading-tight">
              Aceito os <span className="text-primary">termos de uso</span> e <span className="text-primary">política de privacidade</span>
            </label>
          </div>
        ...
        <p className="text-muted-foreground text-xs text-center">
          Já tem conta? <button onClick={() => navigate("/login")} className="text-primary font-semibold hover:underline">Fazer login</button>
        </p>
        <p className="text-muted-foreground text-[10px] text-center opacity-50 pt-2">Powered by EYWA • Experiência Gamificada Inteligente</p>
      </div>
    </motion.div>
  );
};
